...
import static com.vfi.android.payment.presentation.navigation.UIState.UI_STATE_CHOOSE_INSTALLMENT_PROMO;
import static com.vfi.android.payment.presentation.navigation.UIState.UI_STATE_CHOOSE_INSTALLMENT_TERM;
...
public class InstallmentUIFlow2 extends BaseTransFlow implements TransUiFlow {
    ...
    public void navigatorToNext(Context context, UINavigator uiNavigator) {
        ...
        switch (controlData.getStackTopUIState()) {
            ...
            case UI_STATE_CHOOSE_INSTALLMENT_PROMO:
                ui_state_choose_installment_promo(context, controlData);
                break;
            case UI_STATE_CHOOSE_INSTALLMENT_TERM:
                ui_state_choose_intallment_term(context, controlData);
                break;
    ...
    private void ui_state_choose_installment_promo(Context context, UIFlowControlData controlData) {
        jumpToState(context,UI_STATE_CHOOSE_INSTALLMENT_TERM,  ChooseInstallmentTermActivity.class);
    }

    private void ui_state_choose_intallment_term(Context context, UIFlowControlData controlData) {
        if (controlData.isManualInput()) {
            jumpToState(context,UI_STATE_INPUT_CARD_EXPIRY_DATE,  InputCardExpiryDateActivity.class);
        } else if (controlData.isManualInput() || controlData.isSwipeCard()) {
            jumpToState(context,UI_STATE_TRANS_NETWORK_PROCESS,  NetworkProcessActivity.class);
        } else {
